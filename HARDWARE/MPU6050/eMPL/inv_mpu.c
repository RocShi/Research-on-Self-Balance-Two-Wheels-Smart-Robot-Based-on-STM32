...
u8 MPU_DMP_Init(void)
{
	...
	{	 
		if(mpu_set_sensors(INV_XYZ_GYRO|INV_XYZ_ACCEL))  //设置所需要的传感器
			return 1; 
		...
		if(dmp_set_fifo_rate(DEFAULT_MPU_HZ))	//设置DMP输出速率(最大不超过200Hz)
			return 7;   
//		if(Run_SelfTest())		//自检。平衡车项目中需要取消，否则会造成初始化后即为“零态”，导致程序角度平衡点与车身机械平衡点很难重合，极难做到直立，总会偏向一侧
//			return 8;    
...
/* 读取DMP输出的姿态四元数，并转换为姿态角（欧拉角）
 * pitch:俯仰角 精度:0.1°   范围:-90.0° <---> +90.0°
 * roll:横滚角  精度:0.1°   范围:-180.0°<---> +180.0°
 * yaw:航向角   精度:0.1°   范围:-180.0°<---> +180.0°
 */
u8 MPU_DMP_GetData(float *pitch, float *roll, float *yaw)
{
	...
	unsigned char more;
	long quat[4]; 
	if(dmp_read_fifo(gyro, accel, quat, &sensor_timestamp, &sensors,&more))  //读取四元数 
		return 1;	 
...
//		*yaw   = atan2(2*(q1*q2 + q0*q3),q0*q0+q1*q1-q2*q2-q3*q3) * 57.3;	//yaw
		*yaw=0;  //yaw此处用不到，直接将其赋值为0，减少运算，加快程序执行速度
	}else return 2;
	return 0;
}
